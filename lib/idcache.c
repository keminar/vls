...
getuser (uid_t uid)
{
    ...
    if (match == NULL)
    {
        ...
        match = malloc (offsetof (struct userid, name) + strlen (name) + 1);
        match->id.u = uid;
...
getuidbyname (const char *user)
{
  ...
  if (!pwent && strspn (user, digits) < strlen (user))
    ...
    tail = malloc (offsetof (struct userid, name) + strlen (user) + 1);
...
getgidbyname (const char *group)
{
  ...
  if (!grent && strspn (group, digits) < strlen (group))
    ...
    tail = malloc (offsetof (struct userid, name) + strlen (group) + 1);
